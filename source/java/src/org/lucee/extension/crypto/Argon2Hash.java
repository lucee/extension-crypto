...
/**
 * Generates an Argon2 password hash with OWASP-recommended defaults.
 *
 * Usage:
 *   hash = Argon2Hash( "password" )
 *   hash = Argon2Hash( "password", "argon2id", 1, 19456, 2 )
 *
 * Defaults: argon2id, parallelism=1, memory=19456 KB (~19MB), iterations=2
 * These follow OWASP recommendations for password hashing.
 *
 * Replaces GenerateArgon2Hash() which is retained as a deprecated alias
 * with the old extension-argon2 defaults (argon2i, memory=8, iterations=1).
 */
public class Argon2Hash extends BIF {
	...
	public static final String DEFAULT_VARIANT = "argon2id";
	...
	public static String call( PageContext pc, String input, String variant, Number parallelismFactor,
		...
		return generate( pc, input, variant, parallelismFactor, memoryCost, iterations );
	}

	/**
	 * Core implementation shared by Argon2Hash and GenerateArgon2Hash.
	 * Each caller supplies its own defaults before calling this method.
	 */
	public static String generate( PageContext pc, String input, String variant, Number parallelismFactor,
								   Number memoryCost, Number iterations ) throws PageException {
		try {
			// Parse parameters
	...
	static int parseVariant( String variant ) throws PageException {
	...
	static String variantName( int type ) {
		switch ( type ) {
	 ...
	 */
	static String formatHash( int type, int memory, int iterations, int parallelism, byte[] salt, byte[] hash ) {
	...
	public Object invoke( PageContext pc, Object[] args ) throws PageException {
		...
		if ( args.length < 1 ) {
			throw eng.getExceptionUtil()
				.createFunctionException( pc, "Argon2Hash", 1, "input", "Input is required", null );
		}
