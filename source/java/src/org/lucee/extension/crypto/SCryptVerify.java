 ...
 *
 * Usage:
 *   isValid = SCryptVerify( "password", hash )
 *   isValid = SCryptVerify( "password", hash, true )  // throws on invalid hash format
 *
 * Replaces VerifySCryptHash() which is retained as a deprecated alias.
 */
public class SCryptVerify extends BIF {
	...
	/**
	 * Core implementation shared by SCryptVerify and VerifySCryptHash.
	 */
	...
	private static ParsedHash parseHash( String hash ) {
		...
		for ( String param : params ) {
			...
			switch ( kv[0] ) {
				case "ln":
					result.n = 1 << Integer.parseInt( kv[1] );
					break;
				case "r":
	...
	public Object invoke( PageContext pc, Object[] args ) throws PageException {
		...
		if ( args.length < 2 ) {
			throw eng.getExceptionUtil()
				.createFunctionException( pc, "SCryptVerify", 2, "hash", "Input and hash are required", null );
		}
