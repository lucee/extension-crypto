...
/**
 * Generates a Blake2s hash.
 *
 * Blake2s is optimized for 32-bit platforms and embedded systems.
 * Smaller output size than Blake2b (max 32 bytes vs 64 bytes).
 *
 * Usage:
 *   hash = GenerateBlake2sHash( "data" )
 *   hash = GenerateBlake2sHash( "data", 32 )           // output length in bytes (1-32)
 *   hash = GenerateBlake2sHash( "data", 32, keyBytes ) // keyed mode (MAC)
 *
 * Returns: hex-encoded hash string
 */
public class GenerateBlake2sHash extends BIF {

	private static final long serialVersionUID = 1L;
	private static final int DEFAULT_OUTPUT_LENGTH = 32;  // 256 bits
	private static final int MAX_OUTPUT_LENGTH = 32;      // 256 bits max for Blake2s
		...
		try {
			...
			if ( key != null ) {
				...
				if ( keyBytes.length > 32 ) {
					throw eng.getExceptionUtil().createApplicationException(
						"Key length must be at most 32 bytes for Blake2s" );
				}
			}
			...
			if ( keyBytes != null ) {
				digest = new Blake2sDigest( keyBytes, outLen, null, null );
			} else {
				digest = new Blake2sDigest( outLen * 8 );  // constructor takes bits
			}
	...
	public Object invoke( PageContext pc, Object[] args ) throws PageException {
		if ( args.length < 1 ) {
			throw CFMLEngineFactory.getInstance().getExceptionUtil()
				.createFunctionException( pc, "GenerateBlake2sHash", 1, "input", "Input is required", null );
		}
